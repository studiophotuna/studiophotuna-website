...
serve(async (req) => {
  ...
  try {
    ...
    // A discount code, if supplied, is validated (but not yet redeemed --
    // that only happens once payment actually clears, in the matching
    // webhook/capture function) and applied to the charge amount here.
    let discountCodeId: string | null = null;
    if (discount_code && typeof discount_code === "string" && discount_code.trim()) {
      const { data: discount, error: discountErr } = await supabaseAdmin
        .from("discount_codes")
        .select("*")
        .eq("code", discount_code.trim().toUpperCase())
        .maybeSingle();
      if (discountErr) throw discountErr;
      if (!discount) return json({ error: "Invalid discount code." }, 400);
      ...
      amount = discount.discount_type === "percent"
        ? Math.max(0, Math.round(amount * (1 - discount.discount_value / 100)))
        : Math.max(0, Math.round(amount - discount.discount_value));
    ...
    await supabaseAdmin
      .from("licenses")
      .upsert(
        { user_id: user.id, payment_provider: provider, payment_session_id: result.sessionId, pending_billing: billing, pending_discount_code_id: discountCodeId, state: "pending_verification" },
        { onConflict: "user_id" }
      );
