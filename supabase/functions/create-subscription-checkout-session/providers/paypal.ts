// providers/paypal.ts -- PayPal Orders (v2) for a Pro plan subscription
// period (no auto-renewal -- see index.ts header comment).
//
...
// capture-subscription-payment/index.ts, triggered from the app_success
// page when it detects that `token` param.
//
  ...
  const res = await fetch(`${apiBase()}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      ...
      purchase_units: [{
        ...
        custom_id: opts.billing,
        description: opts.description,
        amount: { currency_code: "PHP", value: opts.amount.toFixed(2) },
      }],
      application_context: {
