...
/* ── Printable payment receipt ────────────────────────────────────────────
   Kept byte-for-byte in step with photuna-app/src/lib/receiptDocument.js: the
   receipt an operator prints from the website and the one they print from the
   app have to be the same document. Printed from an iframe, which prints only
   its own content — the page's own stylesheet never reaches it. */
...
function formatReceiptDate(iso, withTime = false) {
  if (!iso) return "—";
 ...
 */
function buildReceiptHtml({ receipt, billedTo, billedName, logoDataUrl }) {
  ...
  const brandMark = logoDataUrl
    ? `<img src="${escReceipt(logoDataUrl)}" alt="Studio Photuna" class="logo" />`
...
<head>
<meta charset="utf-8" />
<title>Receipt ${escReceipt(receipt.receipt_number)} — Studio Photuna</title>
<style>
  ...
  <div class="sheet">
    ...
    <div class="head">
      ...
      <div class="docmeta">
        ...
        <div class="docno">${escReceipt(receipt.receipt_number)}</div>
    ...
    <div class="panels">
      <div class="panel">
        ...
        <div class="value">
          ${billedName ? `<strong>${escReceipt(billedName)}</strong><br />` : ""}
          ${escReceipt(billedTo || "—")}
        </div>
      </div>
      <div class="panel">
        <div class="label">Date paid</div>
        <div class="value">${escReceipt(formatReceiptDate(receipt.paid_at, true))}</div>
      </div>
      <div class="panel">
        <div class="label">Paid with</div>
        <div class="value">${escReceipt(receipt.method || receipt.provider || "—")}</div>
      </div>
    </div>
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>
            <strong>${escReceipt(receipt.description || "Photuna subscription")}</strong>
            <span class="sub">Subscription</span>
          </td>
          <td>${escReceipt(period)}</td>
          <td class="right amount">${escReceipt(total)}</td>
        </tr>
      </tbody>
    </table>
    ...
    <div class="totals">
      <div class="box">
        ...
        <span class="v amount">${escReceipt(total)}</span>
      </div>
    </div>

    <div class="note">
      <strong>Reference:</strong> ${escReceipt(receipt.reference || "—")}<br />
      This is a payment receipt for your records. It is not a BIR Official Receipt.<br />
      Questions about this payment? Email support@studiophotuna.com and quote ${escReceipt(receipt.receipt_number)}.
    </div>
...
// On the website the logo is a normal asset on the same origin, so it needs no
// inlining; the app has to embed it because a print window has no origin.
const RECEIPT_LOGO_URL = location.origin + "/logo-dark.png";

function printReceiptDocument(receiptNumber) {
  const r = _receiptsById[receiptNumber];
  if (!r) return;

  const html = buildReceiptHtml({
    receipt: r,
    billedTo: (window.currentSupabaseUser && window.currentSupabaseUser.email) || "",
    billedName: (currentProfile && (currentProfile.company || currentProfile.full_name)) || "",
    logoDataUrl: RECEIPT_LOGO_URL,
  });
  ...
  const done = () => { try { frame.remove(); } catch (_) {} };
  frame.onload = () => {
    // Give the logo a moment to decode; printing before it does drops it.
    setTimeout(() => {
      try {
        frame.contentWindow.focus();
        frame.contentWindow.print();
      } catch (err) {
        console.warn("Could not print the receipt", err);
      }
      // Chrome's print dialog is modal, so the frame outlives it either way.
...
function openReceiptModal(receiptNumber) {
  ...
  if (printBtn) printBtn.onclick = () => printReceiptDocument(receiptNumber);
